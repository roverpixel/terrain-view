...
* **Edge Interpolation / Non-Rectangular Data:** If dealing with non-rectangular DEMs and Orthos (like orbital passes with NoData edges), ensure the DEM's edges are filled/extrapolated outwards using `gdal_fillnodata.py`. This ensures the deck.gl terrain mesh stays physically flat or appropriately sloping underneath the transparent regions of the orthoimage, preventing vertical "walls" that block oblique views of edge depressions.
